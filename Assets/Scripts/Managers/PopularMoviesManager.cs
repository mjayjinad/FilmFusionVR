...
public class PopularMoviesManager : MonoBehaviour
{
    ...
    private List<Models.MovieData> popularMovieData = new List<Models.MovieData>();
    ...
    public Transform popularMoviesParent;
    ...
    private void LoadPopularMoviesUI()
    {
        ...
        for (int i = 0; i < popularMovieData.Count; i++)
        {
            ...
            movieDataButton.SetActive(true);
    ...
    private IEnumerator GetPopularMovies()
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Get("https://api.themoviedb.org/3/discover/movie?include_adult=false&include_video=false&language=en-US&page=" + pageNumber + "& sort_by=popularity.desc"))
        {
            ...
            {
                Models.MovieData popular = new Models.MovieData();
                popular.spriteURL = jsondata["results"][i]["poster_path"];
                popular.name = jsondata["results"][i]["title"];
                popular.releaseDate = jsondata["results"][i]["release_date"];
                popular.movieRating = jsondata["results"][i]["vote_average"];
                popular.movieID = jsondata["results"][i]["id"];
                popularMovieData.Add(popular);
            }
